...
contract TradeSlippage is OwnableUpgradeable {
     ...
     */
    function getTradeType(address from, address to, uint256, address token)
        public
        view
        returns (bool isBuy, bool isSell, bool isAddLiquidity, bool isRemoveLiquidity, uint256 rO, uint256 rT)
    {
        ...
        if (isToPair) {
            (uint256 rOther, uint256 rThis,,) = getReserves(to, token);
            // Pair balances are externally mutable via direct token transfers, so they cannot
            // safely prove add-liquidity intent. Treat pair-bound transfers as sells; liquidity
            // providers that need fee exemption should be whitelisted or routed through a
            // controlled liquidity manager.
            isSell = true;
            return (isBuy, isSell, isAddLiquidity, isRemoveLiquidity, rOther, rThis);
        }
    }
